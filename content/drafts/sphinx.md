...
Now that we understand _what_ data we need to communicate to each node on a 
route, it’s time to dive into exactly how we package up this data so that we 
leak as little information about the payment to hops along the path. This is 
done using the Sphinx packet construction which ensures a few things:

Each hop on the path will only know the previous hop and the next hop on the 
path. Each hop is only able to decrypt the payload that is meant for it.
...
Diffie-Hellman key exchange is a simple way for two private key owning entities 
to derive a secret shared key between them. All that the two parties need to do 
is exchange public keys.
...
that this is indeed a shared secret key, ask your self if a third party with 
...
In the sphinx packet construction, the onion packet creator, Alice, will use a 
shared key between a public key that she owns and the public key’s of each node 
on the path.
...
public key pair for each hop on the packet and will use that pair to derive the 
...
hop will then be able to use that ephemeral key along with it’s shared secret 
...
A Hash-Based Message Authentication Code (or HMAC) is a message authentication 
technique. Basically, a party with a private-public key pair, can use the 
private key along with the hash of a message they would like to sign to produce 
a 32 byte “signature” called an HMAC. Then any other party can be given the 
message, the HMAC and the signer’s public key and can verify that the owner of 
the private key corresponding to the given public key did in fact create this 
HMAC.
...
The example used in this post continues off where the last post ended off. 
...
payloads are put into the onion packet, they are prefixed with a length and post 
fixed with a 32 HMAC (more on HMACs later). 

![](/onion/5-payloads.png#center)

Alice now goes a ahead and generates the `session_key` and uses this to derive 
the chain of ephemeral keys. 
...
First, Alice generates 1300 bytes pseudo random padding using the session key:
...
HMAC is made to be an empty set of zero bytes and it serves as a signal to Dave 
...
Alice then users her shared key with Dave, $ss_{AB}$ along with the `rho` 
...
Charlie, $s_{AC}$ to derive a pseudo random bytes stream which she XORs with 
...
secret key $ss_{AB}$, he is able to derive the same pseudo random bytes stream 
...
You may have already picked up an issue here… take a moment to see if you can
see the issue. It will become more when we dive into Charlie’s verification 
process below. For now, let’s assume Bob hands this over to Charlie. 

### Charlie peels a layer

Charlie receives the packaged onion from Bob and does a similar verification 
process: He first checks to see if the HMAC is valid give the payload. Ok this 
...
For the sake of the example _and_ so we can find out where else things might 
...
derive a shared secret between him and Alice. He then uses this to derive the 
ephemeral key for David too.

![](/onion/17-charlie_keys.png#center)

Charlie uses the shared secret key to decrypt the packet similarly to how Bob 
did it. He then can read his payload and reconstruct the packet for David. 

![](/onion/18-peel_charlie_2.png#center)

### Dave peels a layer

David then gets this packet and attempts to do the HMAC verification. Once 
again, this will fail since the packet contents does not match the packet 
contents that Alice used when creating the HMAC. But again, we know what it 
...
David will see that the HMAC sent to him is a zero byte array. He therefor knows
that he is the last hop for this packet. 

# Second attempt at wrapping the onion. 

Ok ok so we’ve made some mistakes but we have also learned some lessons. Now 
that we know what each packet needs to look like at the time of computing the 
HMACs, we can now redo the onion wrapping process correctly.

Let’s start by taking a look at the look of the packet when David gets it 
(before decryption):
...
which psuedo random bytes streams and which sections of those bytes streams are 
...
Ok cool! So let’s try this again shall we?
...
Here is where things change! We now that before Alice hashes Dave’s payload to 
...
it doesn’t seem that we do! Due to the properties of XOR, XORing the packet 
...
first pass packet. The only difference that now each node will be able to see 
that each HMAC that it is given is correct. 
